...
class CMDRegister : ListenerAdapter() {
    ...
    private fun serverInfoCMD(commandData: MutableList<CommandData>) {
        try {
            commandData.add(
                Commands.slash(
                    "mcinfo", ColorUtils().convert(
                        Language().directFormat(
                            "Sends a helpful overview about the Minecraft Server",
    ...
    private fun whitelistCMD(commandData: MutableList<CommandData>) {
        val data1 = SubcommandData(
            "forceadd", ColorUtils().convert(
                Language().directFormat(
        ...
        val data2 = SubcommandData(
            "forceremove", ColorUtils().convert(
                Language().directFormat(
                    "Gives you a menu, where you can select one account, which will be removed from the whitelist",
                    "Gibt ein Menu zurück, in dem du einen Account auswählen kannst, welcher von der whitelist entfernt wird"
                )
            )
        )

        val data3 = SubcommandData(
            "check", ColorUtils().convert(
                Language().directFormat(
        ...
        options1.add(
            OptionData(
                OptionType.STRING, "mc_name", ColorUtils().convert(
                    Language().directFormat(
                        "Enter a Minecraft InGame Name to whitelist this Name.",
                        "Schreibe hier, welcher Minecraft Account zur whitelist hinzugefügt werden soll"
                    )
                ), true
            )
        )
        ...
        options1.add(
            OptionData(
                OptionType.USER, "dc_name", ColorUtils().convert(
                    Language().directFormat(
                        "When you want to link an Discord Account to an MC Account, select here the Account of the User",
                        "Wenn du einen Discord Account linken willst, setze ihn hiermit fest"
                    )
                ), false
            )
        )
        ...
        options2.add(
            OptionData(
                OptionType.INTEGER, "site", ColorUtils().convert(
                    Language().directFormat(
                        "Enter the site you want to see from the whitelist (25 players per site), leave blank for site one",
                        "Bitte wähle die gewünschte Seite hier aus (Pro Seite 25 Einträge). Leer lassen für Seite Eins"
                    )
                ), false
            )
        )
        ...
        options3.add(
            OptionData(
                OptionType.STRING, "name", ColorUtils().convert(
                    Language().directFormat(
                        "Input a Name from Minecraft to check it",
                        "Setzte hier einen Minecraft Namen fest, welchen du überprüfen willst"
                    )
                ), true
            )
        )
        ...
        try {
            commandData.add(
                Commands.slash(
                    "whitelist", ColorUtils().convert(
                        Language().directFormat(
                            "Sends a Whitelist Request to the Minecraft Server",
        ...
        try {
            ...
            commandData.add(
                Commands.slash(
                    "help", ColorUtils().convert(
                        Language().directFormat(
                            "Sends a helpful overview about the YVtils SMP Plugin Features and Guides",
